...
RC LogicalPlanGenerator::create_plan(SelectStmt *select_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  auto project_oper = make_unique<ProjectLogicalOperator>
    (std::move(select_stmt->query_expressions()),select_stmt->order_by_);
  if (*last_oper) {
...
RC LogicalPlanGenerator::create_plan(FilterStmt *filter_stmt, unique_ptr<LogicalOperator> &logical_operator)
{
  ...
  for (const FilterUnit *filter_unit : filter_units) {
    ...
    unique_ptr<Expression> left=std::unique_ptr<Expression>(filter_obj_left.expr);
    //std::move(filter_obj_left.expr);//(filter_obj_left.is_attr==2?
                                    // std::move(*filter_obj_left.expr):(filter_obj_left.is_attr==1
                                    // ? static_cast<Expression *>(new FieldExpr(filter_obj_left.field))
                                    // : static_cast<Expression *>(new ValueExpr(filter_obj_left.value))));

    unique_ptr<Expression> right=std::unique_ptr<Expression>(filter_obj_right.expr);
    //(filter_obj_right.is_attr==2?
                                    // std::move(*filter_obj_right.expr):(filter_obj_right.is_attr==1
                                    //  ? static_cast<Expression *>(new FieldExpr(filter_obj_right.field))
                                    //  : static_cast<Expression *>(new ValueExpr(filter_obj_right.value))));
    //filter_obj_left.expr=nullptr;
    //assert(filter_obj_left.expr!=nullptr);
    //assert(filter_obj_right.expr!=nullptr);
    //filter_obj_left->expr=nullptr;
